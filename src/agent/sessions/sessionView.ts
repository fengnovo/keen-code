      ...
      for (const runFile of runFiles) {
        ...
        try {
          ...
          if (firstEvent.type === "session_start") {
            firstInput = String((firstEvent.data as { userInput?: string }).userInput || "").slice(0, 50);
          }
          const lastEvent: SessionEvent = JSON.parse(lines[lines.length - 1]);
          if (lastEvent.type === "session_end") {
            lastOutput = String((lastEvent.data as { finalAnswer?: string }).finalAnswer || "").slice(0, 50);
          }
        } catch {
          // JSON 解析失败，忽略
        }
        ...
        console.log(`    事件数: ${eventCount} | 修改时间: ${stat.mtime.toLocaleString()}`);
 ...
 * @param runId 运行 ID
 */
export async function showSessionTree(sessionId: string, runId: string): Promise<void> {
  ...
  try {
    ...
    for (const event of events) {
      ...
      const indent = "  ".repeat(event.type === "turn_start" || event.type === "turn_end" ? 0 : 1);
